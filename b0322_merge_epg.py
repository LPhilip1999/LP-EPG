...
def fix_timestamp(ts):
    """Handles standard XMLTV, Bongo +0600, and compact digits[cite: 1]."""
    if not ts: return ts
    digits = re.sub(r'\D', '', ts)
    if len(digits) >= 14:
        main_time = digits[:14]
        # Preserve existing offset (+0600) if present [cite: 1]
...
def fix_yachting_time(date_str, time_str):
    """Specific fix for Yachting TV separate attributes [cite: 1-2]."""
    try:
...
def merge_epg():
    ...
    merged_root.set("generator-info-name", "Metax-Universal-V11")
    ...
    for line in lines:
        ...
        try:
            ...
            # --- 1. BLOOMBERG JSON HANDLING [cite: 5-7] ---
            if "bloomberg" in url.lower() or url.endswith(".json"):
                ...
                for item in data:
                    ...
                    prog = ET.SubElement(merged_root, "programme", channel=target_id, 
            ...
            # --- 2. XML HANDLING (HI LIFE, YACHTING, SOFAST, BONGO) [cite: 8-15] ---
            else:
                source_root = ET.fromstring(response.content)
                chan_elem = ET.SubElement(merged_root, "channel", id=target_id)
                ET.SubElement(chan_elem, "display-name").text = display_name
                
                # Check for both 'programme' and 'program' (Hi Life Fix) [cite: 9]
                for prog in source_root.findall(".//programme") + source_root.findall(".//program"):
                    ...
                    if xmltv_start and xmltv_stop:
                        ...
                        # CLEANED TAG MAPPING: Removed Category, Genre, and shortSynopsis [cite: 10-14]
                        tags_to_find = {
    ...
    print("Success! EPG updated (Bongo specific tags removed).")
